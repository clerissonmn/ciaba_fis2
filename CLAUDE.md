# CLAUDE.md — Orquestrador do Curso FIS-02

Este projeto publica o material do curso de Física (FIS-02 — Escola de formação de
...
- **RAs de FIS-02:** vêm de `FIS_2_RA.md`. Não inventar.
...
## 5. CONVENÇÕES QUARTO (Apêndice Q)

- Estrutura do projeto e nomes: `aula-NN-slug.qmd`, `lista-NN.qmd` (Q2). `_site/` é gerado — nunca editar nem comitar.
- Cada `.qmd` de aula segue o template de 5 partes do Mestre §10; códigos `[RAx][By]` no `categories` do YAML.
...
```
foto do quadro → P5 (transcreve) → professor revisa → aula-NN.qmd
